...
//#include "UART_interface.h"  // dont use this layer as i already using it in bluetooth module
...
int main(void){
	// intialization lcd
	 ...
	 str_UartConfg_t Data_UART={9600,UART_8_BIT_MODE,UART_INTERRUPT};
	St_SPICfg_t Data_SPI={SPI_MASTER,SPI_MASTER_FREQUENCY_64,SPI_INTERRUPT};
	// intialization UART_HAL_BLUETOOTH_init
	 HAL_BLUETOOTH_init(&Data_UART);
	 // intialization SPI
	 ...
	 HAL_LCD_voidClearAll();
	 while(1){
	   HAL_BLUETOOTH_RecieveCallBack(send_function);
	     } }
             /****************************/
void send_function(uint8_t value){
	   HAL_LCD_MoveCursor(0,0);
